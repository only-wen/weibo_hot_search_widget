...
- **现代简约卡片设计**：
  - 半透明白色毛玻璃卡片背景（`rgba(255, 255, 255, 0.92)`）配以 `24dp` 圆角。
  - 微悬浮阴影感，适合各种桌面壁纸。
  - 优化了标题栏占比，确保词条区域空间最大化，显示更饱满。
- **直观的热搜榜单**：
  - 支持显示最新的前 8 条热搜词条。
  - 前 3 名配以专属的彩色徽标（红、橙、黄），4-8 名配以灰色徽标，重点突出。
  - 支持分割线区分，视觉层次分明。
- **高效与流畅性**：
...
1. **直接安装 APK**：
   下载项目根目录下的 [weibo-hot-search-widget.apk](./weibo-hot-search-widget.apk) 移至安卓手机中安装。
2. **启动应用**：
   安装完成后，首次打开主程序（加载主页面）以初始化网络请求并保存本地缓存。
3. **添加小组件**：
   回到手机桌面，长按空白处选择“小组件/窗口小部件”，找到“微博热搜”并将其拖放到桌面合适位置即可。
